...
public sealed record RoleDefinitionRecord
{
    ...
    /// <summary>
    /// Microsoft's ROLE-level privileged flag, from the BETA unifiedRoleDefinition: the
    /// role contains at least one sensitive action, i.e. it is an escalation path. This is
    /// NOT the same question as excess risk — a role can carry very little excess and
    /// still be one.
    ///
    /// null = NOT READ (a v1.0 sync, or the beta enrichment pass did not run). Never read
    /// null as false: an unenriched catalog would otherwise rank every role as safe.
    /// Nullable also keeps an existing catalog.json readable — the field deserializes to
    /// null when absent, so no re-sync is required to build.
    /// </summary>
    public bool? IsPrivilegedRole { get; init; }

    /// <summary>
    /// Beta only: which principal types may hold this role. A flags enum, serialised by
    /// Graph as ONE comma-separated string ("user,servicePrincipal"), not an array.
    /// null when the beta enrichment has not run.
    /// </summary>
    public string? AllowedPrincipalTypes { get; init; }

    /// <summary>
    /// Beta only, and ABSENT FROM MICROSOFT'S DOCUMENTED PROPERTY TABLE despite appearing
    /// in real responses. Read precisely because it is the likeliest carrier of
    /// AU-scopability, which no documented field expresses — resourceScopes looks like that
    /// signal and is not. null when the beta enrichment has not run.
    /// </summary>
    public string? AssignmentMode { get; init; }

    /// <summary>Beta only: Microsoft's own grouping for the role. null when unread.</summary>
    public string? Categories { get; init; }

    /// <summary>
    /// Beta only: fuller prose than <see cref="Description"/>. Describes the ROLE, so it is
    /// still not a permission's meaning — display only, never an input to recommendation.
    /// </summary>
