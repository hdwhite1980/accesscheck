...
var configPath = new[]
{
    ...
    Path.Combine("src", "AccessLens.Cli", "appsettings.json"),
    "appsettings.sample.json",
    Path.Combine(AppContext.BaseDirectory, "appsettings.sample.json"),
    Path.Combine("src", "AccessLens.Cli", "appsettings.sample.json")
}.FirstOrDefault(File.Exists) ?? "appsettings.json";

if (args.Length == 0)
{
    Console.WriteLine("""
        AccessLens CLI — least-privilege access broker (pipeline harness)

        usage:
          accesslens demo "<function description>"     offline: sample catalog + demo suggester
          accesslens set-key                           store the GenAI API key (DPAPI)
          accesslens sync                              sync ALL RBAC providers from Graph
          accesslens recommend "<function>"            GenAI suggest -> validate -> show verdicts
          accesslens approve <principalId> "<function>" [P14D] [--eligible]
                                                       full flow incl. Graph execution after Y/N
          accesslens housekeeping                      GC roles + remove expired direct grants
        """);
    return 0;
}
...
switch (command)
{
    case "demo":
    {
        ...
        var suggestion = await provider.SuggestAsync(function, catalog.Roles);
    ...
    case "set-key":
    {
        ...
        {
            Console.WriteLine("set-key uses Windows DPAPI; run on the target machine.");
            return 1;
        }
        ...
        foreach (var po in actionable)
        {
            ...
            if (RbacProviders.DerivedRoleCapable.Contains(po.Provider))
            {
                ...
                Console.WriteLine(RbacProviders.DisplayName(po.Provider) +
                    " grants run as PowerShell — use the AccessLens app (script shown for review), " +
            ...
            if (po.Provider == RbacProviders.Directory)
            {
                var plan = new AssignmentPlan
                {
                    ...
                    Justification = "AccessLens least-privilege grant: " + function,
            ...
            else
            {
                var assignmentId = await executor.AssignMultiAsync(
                    ...
                    "AccessLens least-privilege grant: " + function);
    ...
    case "housekeeping":
    {
        ...
        // 2. GC AccessLens-created roles with no remaining assignments
        Console.WriteLine("Re-syncing catalog for role GC...");
        var (catalog, _) = await new CatalogSync(graph).SyncAllAsync(Console.WriteLine);
        catalog.Save(catalogPath);
        foreach (var role in catalog.Roles.Where(r => r.IsAccessLensCreated))
        {
...
{
    string? key = OperatingSystem.IsWindows() ? SecretStore.Load(cfg.Ai.ApiKeyName) : null;
    if (string.IsNullOrEmpty(key))
        throw new InvalidOperationException(
            "No GenAI key stored. Run: accesslens set-key   (config: " + configPath + ")");
